...
const TRUSTED_ISSUERS: Record<string, string> = {
...
export function signLocalJwt(
  claims: Omit<JwtClaims, "aud" | "exp"> & Partial<Pick<JwtClaims, "aud" | "exp">>
): string {
...
export function verifyJwt(token: string): JwtClaims {
  ...
  const header = JSON.parse(decodeB64url(encodedHeader).toString("utf8")) as { alg?: string };
  if (header.alg !== "HS256") throw new Error("Unexpected algorithm");

  const claims = JSON.parse(decodeB64url(encodedPayload).toString("utf8")) as JwtClaims;
